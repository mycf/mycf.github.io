...
在上篇文章[《Java并发编程之锁机制之AQS(AbstractQueuedSynchronizer)》](https://juejin.cn/post/6844903689375711239 "https://juejin.cn/post/6844903689375711239")中我们了解了整个AQS的内部结构，与其独占式与共享式获取同步状态的实现。但是并没有详细描述线程是如何进行阻塞与唤醒的。我也提到了线程的这些操作都与`LockSupport`工具类有关。现在我们就一起来探讨一下该类的具体实现。
...
| 带Blocker的方法                          | 作用                                                                                   | 不带Blocker的方法     | 作用 |
| ---------------------------------------- | -------------------------------------------------------------------------------------- | --------------------- | ---- |
| park(Object blocker)                     | 阻塞当前线程，如果调用unpark(Thread thread)方法或者当前线程被中断，才能从park(方法返回 | park()                | 同左 |
| parkNanos(Object blocker, long nanos)    | 阻塞当前线程，最长不超tnanos纳秒，返回条件在park(Object blocker)的基础上添加了超时返回 | parkNanos(long nanos) |      |
| parkUntil(Object blocker, long deadline) | 阻塞当前线程，直到deadlinel parkUntil(long deadline)                                   | parkUntil(long deadline) |      |
...
![blocker.png](https://p1-jj.byteimg.com/tos-cn-i-t2oaga2asx/gold-user-assets/2018/11/2/166d3a3607a75617~tplv-t2oaga2asx-jj-mark:3024:0:0:0:q75.png)
...
那么我们再回到`setBlocker(Thread t, Object arg)`方法，当我们获取到`parkBlocker`字段在其对象内存偏移量后， 接着会调用`U.putObject(t, PARKBLOCKER, arg);`，该方法有三个参数,第一个参数是操作对象，第二个参数是内存偏移量，第三个参数是实际存储值。该方法理解起来也很简单，就是`操作某个对象中某个内存地址下的数据`。那么结合我们上面所讲的。该方法的实际操作结果如下图所示：

![blocker_set.png](https://p1-jj.byteimg.com/tos-cn-i-t2oaga2asx/gold-user-assets/2018/11/2/166d3a3607c56221~tplv-t2oaga2asx-jj-mark:3024:0:0:0:q75.png)
...
![parker.png](https://p1-jj.byteimg.com/tos-cn-i-t2oaga2asx/gold-user-assets/2018/11/2/166d3a3607c8b866~tplv-t2oaga2asx-jj-mark:3024:0:0:0:q75.png)
...
![linux方法.png](https://p1-jj.byteimg.com/tos-cn-i-t2oaga2asx/gold-user-assets/2018/11/2/166d3a3607d4ba8c~tplv-t2oaga2asx-jj-mark:3024:0:0:0:q75.png)
